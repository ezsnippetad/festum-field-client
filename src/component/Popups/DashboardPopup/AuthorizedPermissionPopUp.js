...
import { friendsRequest, friendsRequestsSearch, getSingleFriend, setMyNewFriedList, useFriendRequestFromId, useMyFriendsList, useSingleFriend, } from "../../../redux/Slice/requestSlice";
import { useCurrentChatUser } from "../../../redux/Slice/chatSlice";

export default function AuthorizedPermissionPopUp({ handleClose }) {
  const { id } = useParams()
  const [isLoading, setLoading] = useState(false)
  const userProfile = useProfileGets()
  const dispatch = useDispatch()
  const [values, setValues] = useState({})
  const myFriend = useSingleFriend()
